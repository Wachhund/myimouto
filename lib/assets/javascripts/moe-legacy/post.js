Post = {
  ...
  update_batch: function(posts, finished) {
    ...
    posts.each(function(post) {
      $H(post).each(function(pair2) {
        var s = "post[" + i + "][" + pair2.key + "]=" + window.encodeURIComponent(pair2.value); // Pop.
  ...
  flag: function(id, finished) {
    /* Build flag dialog from FLAG_REASONS config */
    var reasons = (typeof FLAG_REASONS !== "undefined") ? FLAG_REASONS : [];

    /* Remove any existing dialog */
    var existing = $("flag-dialog-overlay");
    if (existing) existing.remove();

    /* Build overlay + dialog HTML */
    var html = '<div id="flag-dialog-overlay" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;">' +
      '<div style="background:#2a2a2a;color:#ddd;border:1px solid #555;border-radius:6px;padding:1.5em;min-width:360px;max-width:480px;box-shadow:0 4px 20px rgba(0,0,0,0.5);">' +
        '<h3 style="margin:0 0 1em 0;font-size:1.1em;">Flag Post</h3>' +
        '<div style="margin-bottom:0.8em;">' +
          '<label for="flag-category" style="display:block;margin-bottom:0.3em;">Category:</label>' +
          '<select id="flag-category" style="width:100%;padding:0.4em;background:#1a1a1a;color:#ddd;border:1px solid #555;">';

    html += '<option value="">-- Select reason --</option>';
    for (var i = 0; i < reasons.length; i++) {
      html += '<option value="' + reasons[i].key + '">' + reasons[i].label + '</option>';
    }

    html += '</select></div>' +
        '<div id="flag-reason-wrap" style="display:none;margin-bottom:0.8em;">' +
          '<label for="flag-reason" style="display:block;margin-bottom:0.3em;">Reason:</label>' +
          '<textarea id="flag-reason" rows="3" style="width:100%;padding:0.4em;background:#1a1a1a;color:#ddd;border:1px solid #555;box-sizing:border-box;resize:vertical;"></textarea>' +
        '</div>' +
        '<div id="flag-parent-wrap" style="display:none;margin-bottom:0.8em;">' +
          '<label for="flag-parent-id" style="display:block;margin-bottom:0.3em;">Parent Post ID:</label>' +
          '<input type="text" id="flag-parent-id" style="width:100%;padding:0.4em;background:#1a1a1a;color:#ddd;border:1px solid #555;box-sizing:border-box;" />' +
        '</div>' +
        '<div style="text-align:right;margin-top:1em;">' +
          '<button id="flag-cancel-btn" type="button" style="margin-right:0.5em;padding:0.4em 1.2em;cursor:pointer;">Cancel</button>' +
          '<button id="flag-submit-btn" type="button" style="padding:0.4em 1.2em;cursor:pointer;" disabled="disabled">Submit</button>' +
        '</div>' +
      '</div></div>';
    ...
    var overlay = $("flag-dialog-overlay");
    var categorySelect = $("flag-category");
    var reasonWrap = $("flag-reason-wrap");
    var parentWrap = $("flag-parent-wrap");
    var submitBtn = $("flag-submit-btn");

    /* Helper: find reason config by key */
    function findReason(key) {
      for (var j = 0; j < reasons.length; j++) {
        if (reasons[j].key === key) return reasons[j];
      }
      return null;
    }

    /* Update visible fields based on category selection */
    function updateFields() {
      var cfg = findReason(categorySelect.value);
      if (!cfg) {
        reasonWrap.hide();
        parentWrap.hide();
      ...
      if (cfg.requires_detail) { reasonWrap.show(); } else { reasonWrap.hide(); $("flag-reason").value = ""; }
      if (cfg.requires_parent) { parentWrap.show(); } else { parentWrap.hide(); $("flag-parent-id").value = ""; }
      submitBtn.disabled = false;
    }

    categorySelect.observe("change", updateFields);

    /* Cancel */
    $("flag-cancel-btn").observe("click", function() { overlay.remove(); });
    overlay.observe("click", function(e) { if (e.target === overlay) overlay.remove(); });

    /* Submit */
    submitBtn.observe("click", function() {
      var cat = categorySelect.value;
      if (!cat) return;
      var cfg = findReason(cat);
      var reason = $("flag-reason").value.strip();
      var parent_id = $("flag-parent-id").value.strip();

      if (cfg && cfg.requires_detail && !reason) {
        notice("Please provide a reason.");
        return;
      }
      if (cfg && cfg.requires_parent && !parent_id) {
        notice("Please provide a parent post ID.");
        return;
      }

      var params = { "id": id, "reason_category": cat, "reason": reason };
      if (parent_id) params["parent_post_id"] = parent_id;
      ...
      var complete = function() {
        ...
        if (finished)
          ...
          var e = $("p" + id);
          if (e) e.addClassName("flagged");
        }
      };

      Post.make_request("/post/flag.json", params, complete);
    });
  },
  ...
  apply_blacklists: function() {	
    ...
    Post.posts.each(function(pair) {
      ...
      {
        if(bld)
          thumb.addClassName("javascript-hide").addClassName('blacklisted-post'); // Pop.
        else
          thumb.removeClassName("javascript-hide");
      }
    })
  ...
  init_blacklisted: function(options) {
    ...
    else
    {
      ...
      bl_entries = eval(jQuery.cookie("blacklisted_tags")); // Pop.
    }
