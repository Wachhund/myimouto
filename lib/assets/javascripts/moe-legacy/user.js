User = {
  ...
  set_login: function(username, unused, user_info)
  {
    // login and remember_token cookies are now set server-side with HttpOnly flag
      ...
      if(current_state == "login-confirm-password")
      {
        ...
        else
        {
          ...
          User.create(username, password, null, function(resp) {
            ...
            {
              User.set_login(resp.name, null, resp.user_info);
              User.close(true);
            }
