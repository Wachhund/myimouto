...
trait PostFileMethods
{
     ...
     */
    protected static $create_fake_sample_url = false;
     ...
     * For Import
     */
    public static function get_import_files($dir)
    {
    ...
    public function pretty_file_name($options = [])
    {
    ...
    public function fake_sample_url()
    {
        if (CONFIG()->use_pretty_image_urls) {
            $path = "/data/image/" . $this->md5 . "/" . $this->pretty_file_name(['type' => 'sample']) . '.' . $this->file_ext;
        } else {
    ...
    public function regenerate_images($type, array $options = [])
    {
        if (!$this->image()) {
            return true;
        }
    ...
    {
        if (!CONFIG()->min_mpixels) {
            return false;
        }
        if (empty($this->width)) {
            return false;
        }
        if ($this->width * $this->height >= CONFIG()->min_mpixels) {
            return false;
        }
        return true;
    }
    ...
    {
        return in_array($this->file_ext, ['jpg', 'jpeg', 'gif', 'png']);
    }
    ...
    {
        if ($this->image()) {
            $dim = Moebooru\Resizer::reduce_to(['width' => $this->width, 'height' => $this->height], ['width' => 300, 'height' => 300]);
    ...
    public function generate_sample($force_regen = false)
    {
        if ($this->gif() || !$this->image()) {
            return true;
        } elseif (!CONFIG()->image_samples) {
            return true;
        } elseif (!$this->width && !$this->height) {
            return true;
        } elseif ($this->file_ext == "gif") {
            return true;
        }
    ...
    protected function generate_preview($force_regen = false)
    {
        ...
        $size = Moebooru\Resizer::reduce_to(['width' => $this->width, 'height' => $this->height], ['width' => 300, 'height' => 300]);

        # Generate the preview from the new sample if we have one to save CPU, otherwise from the image.
        if (is_file($this->tempfile_sample_path())) {
            list($path, $ext) = [$this->tempfile_sample_path(), "jpg"];
        } elseif (is_file($this->sample_path())) {
            list($path, $ext) = [$this->sample_path(), "jpg"];
        } elseif (is_file($this->tempfile_path)) {
            list($path, $ext) = [$this->tempfile_path, $this->file_ext];
        } elseif (is_file($this->file_path())) {
            list($path, $ext) = [$this->file_path(), $this->file_ext];
        } else {
            return false;
        }
    ...
    protected function generate_jpeg($force_regen = false)
    {
        ...
        $size = Moebooru\Resizer::reduce_to(['width' => $this->width, 'height' => $this->height], ['width' => CONFIG()->jpeg_width, 'height' => CONFIG()->jpeg_height], CONFIG()->jpeg_ratio);
        try {
