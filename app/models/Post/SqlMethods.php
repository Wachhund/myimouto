...
{
    public static function find_by_tag_join($tag, array $options = [])
    {
        $options = array_merge([
            'order' => "posts.id DESC",
            'limit' => null,
        ...
        if (!$posts) {
            $posts = new ModelCollection([]);
        }

        return $posts;
    }

    public static function generate_sql_range_helper($arr, $field, &$c, &$params = null)
    {
    ...
    public static function generate_sql_escape_helper($array)
    {
    ...
    public static function generate_sql($q, $options = [])
    {
        if (is_array($q)) {
        ...
        $q = array_merge(array_fill_keys(['md5', 'ext', 'source', 'fav', 'user', 'subscriptions', 'rating', 'rating_negated', 'unlocked_rating', 'show_holds', 'shown_in_index', 'exclude', 'related', 'post_id', 'mpixels', 'width', 'height', 'score', 'date', 'change'], null), $q);
        $options = array_merge(array_fill_keys(['pending', 'flagged', 'from_api', 'limit', 'offset', 'count', 'select', 'having'], null), $options);
