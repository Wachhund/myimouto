...
{
    public static function with_implied($tags)
    {
        if (!$tags) {
            return [];
        }
    ...
    public function destroy_and_notify($current_user, $reason)
    {
        ...
        if (!empty($this->creator_id) && $this->creator_id != $current_user->id) {
            ...
            Dmail::create(['from_id' => current_user()->id, 'to_id' => $this->creator_id, 'title' => "One of your tag implications was deleted", 'body' => $msg]);
        }
    ...
    public function approve($user_id, $ip_addr)
    {
        ...
        $implied_tags = implode(' ', self::with_implied([$t->name]));

        foreach (Post::where("id IN (SELECT pt.post_id FROM posts_tags pt WHERE pt.tag_id = ?)", $t->id)->take() as $post) {
            $post->updateAttributes(['tags' => $post->cached_tags . " " . $implied_tags, 'updater_user_id' => $user_id, 'updater_ip_addr' => $ip_addr]);
        }
    }
