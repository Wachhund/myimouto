...
foreach (glob(dirname(__FILE__) . '/Tag/*.php') as $trait) {
    require $trait;
}
...
class Tag extends Rails\ActiveRecord\Base
{
    ...
    public static function find_or_create_by_name($name)
    {
        ...
        $name = str_replace([' ', '`'], ['_', ''], ltrim((strtolower($name)), '-~'));
        ...
        if ($tag) {
            ...
            return self::create(['name' => $name, 'tag_type' => $tag_type, 'is_ambiguous' => $ambiguous]);
        }
    }

    /* } Parse methods { */
    public static function scan_query($query)
    {
        return array_unique(array_filter(explode(' ', strtolower($query))));
    }

    public static function scan_tags($tags)
    {
        return array_unique(explode(' ', str_replace(['%', ','], '', strtolower($tags))));
    }

    public static function select_ambiguous($tags)
    {
        if ($tags) {
    ...
    public static function find_suggestions($query)
    {
    ...
    public static function parse_cast($x, $type)
    {
        if ($type == 'int') {
            $val = (int) $x;
    ...
    public static function parse_helper($range, $type = 'int')
    {
        # "1", "0.5", "5.", ".5":
        # (-?(\d+(\.\d*)?|\d*\.\d+))
        // case range
        if (preg_match('/^(.+?)\.\.(.+)/', $range, $m)) {
            return ['between', self::parse_cast($m[1], $type), self::parse_cast($m[2], $type)];
        } elseif (preg_match('/^<=(.+)|^\.\.(.+)/', $range, $m)) {
            return ['lte', self::parse_cast($m[1], $type)];
        } elseif (preg_match('/^<(.+)/', $range, $m)) {
            return ['lt', self::parse_cast($m[1], $type)];
        } elseif (preg_match('/^>=(.+)|^(.+)\.\.$/', $range, $m)) {
            return ['gte', self::parse_cast($m[1], $type)];
        } elseif (preg_match('/^>(.+)/', $range, $m)) {
            return ['gt', self::parse_cast($m[1], $type)];
        } elseif (preg_match('/^(.+?)|(.+)/', $range, $m)) {
            $items = explode(',', $range);
            ...
            return ['eq', self::parse_cast($range, $type)];
        }
    }

    public static function parse_query($query, $options = [])
    {
        $q = ['include' => [], 'exclude' => [], 'related' => []];
        ...
        foreach (self::scan_query($query) as $token) {
            ...
            if (preg_match('/^(unlocked|deleted|ext|user|sub|vote|-vote|fav|md5|-rating|rating|width|height|mpixels|score|source|id|date|pool|-pool|parent|order|change|holds|pending|shown|limit):(.+)$/', $token, $m)) {
                if ($m[1] == "user") {
                    ...
                    $q['vote'] = [self::parse_helper($vote), $user_id];
                } elseif ($m[1] == "-vote") {
    ...
    public static function compact_tags($tags, $max_len)
    {
    ...
    public static function count_by_period($start, $stop, array $options = [])
    {
        $options['limit'] = !isset($options['limit']) || !ctype_digit((string) $options['limit']) ? 50 : (int) $options['limit'];
        !isset($options['exclude_types']) && $options['exclude_types'] = [];
        $sql = 'SELECT
    ...
    public static function type_name_helper($tag_name) # :nodoc:
    {
    ...
    public static function type_name($tag_name)
    {
    ...
    public static function tag_type_indexes()
    {
    ...
    public static function tag_list_order($tag_type)
    {
        switch ($tag_type) {
    ...
    public static function type_name_from_value($type_value)
    {
    ...
    public static function batch_get_tag_types_for_posts($posts)
    {
        $tags = [];
    ...
    public static function batch_get_tag_types($post_tags)
    {
        $post_tags_key = [];
    ...
    public static function calculate_related_by_type($tag, $type, $limit = 25)
    {
    ...
    public static function find_related($tags)
    {
        if (is_array($tags)) {
            if (count($tags) == 1) {
    ...
    public static function calculate_related($tags)
    {
        if (!$tags) {
            return [];
        }
    ...
    public static function get_summary_version()
    {
        return Rails::cache()->fetch('$tag_version', function () {
            return 0;
        });
    }
    ...
    public static function get_json_summary_no_cache()
    {
    ...
    public static function purge_tags()
    {
    ...
    public static function recalculate_post_count()
    {
    ...
    public static function mass_edit($start_tags, $result_tags, $updater_id, $updater_ip_addr)
    {
        foreach (Post::find_by_tags($start_tags) as $p) {
            ...
            $tags = implode(' ', $tags);
            $p->updateAttributes(['updater_user_id' => $updater_id, 'updater_ip_addr' => $updater_ip_addr, 'tags' => $tags]);
        }
    }

    private static function _type_map($tag_type)
    {
