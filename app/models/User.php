...
class User extends Rails\ActiveRecord\Base
{
    ...
    {
        UserBlacklistedTag::create(['user_id' => $this->id, 'tags' => implode("\r\n", CONFIG()->default_blacklists)]);
    }
    ...
    public static function authenticate_remember_token($name, $token)
    {
        $user = parent::where("lower(name) = lower(?)", $name)->first();
    ...
    public static function authenticate_with_api_key($name, $api_key)
    {
        if (!$name || !$api_key) {
            return null;
        }
    ...
    public static function sha1($pass)
    {
    ...
    public static function fast_count()
    {
    ...
    private static function _find_name_helper($user_id)
    {
        if (!$user_id) {
    ...
    # UserTagMethods {
    # iTODO:
    public function uploaded_tags(array $options = [])
    {
    ...
    public function voted_tags(array $options = [])
    {
        $type = !empty($options['type']) ? $options['type'] : null;
    ...
    public function favorite_post_count($options = [])
    {
    ...
    public static function get_user_level($level)
    {
        static $user_level = [];
    ...
    public static function clear_avatars($post_id)
    {
    ...
    public static function generate_sql($params)
    {
    ...
    protected function callbacks()
    {
        ...
        return [
            ...
            'before_validation' => ['_commit_secondary_languages'],
            'after_save'        => ['_commit_blacklists', 'update_cached_name'],
            'after_create'      => ['_set_default_blacklisted_tags', '_increment_count'],
            'after_destroy'     => ['_decrement_count'],
        ];
    }
    ...
    protected function validations()
    {
        $validations = [
            'name' => [
                ...
                'format'     => ['with' => '/\A[^\s;,]+\Z/', 'on' => 'create', 'message' => 'cannot have whitespace, commas, or semicolons'],
                'uniqueness' => [true, 'on' => 'create'],
            ],
            'password' => [
                'length'       => ['minimum' => 5, 'if' => ['property_exists' => 'password']],
                'confirmation' => true,
            ],
            'language' => [
                'format' => ['with' => '/^([a-z\-]+)|$/'],
        ...
        if (CONFIG()->enable_account_email_activation) {
            $validations['email'] = [
                'presence' => [true, 'on' => 'create', 'if' => ['property_exists' => 'email']],
            ];
        }
        return $validations;
    }
