  ...
  <ul>
    ...
    <?php if ($this->post->image() && $this->post->can_be_seen_by(current_user())) : ?>
      ...
      <li><?php if (array_key_exists("dakimakura", $this->post->tags()) && !current_user()->is_contributor_or_higher()) :
          ...
          echo $this->linkTo(($this->post->has_sample() ? $this->t('.download.larger') : $this->t('.download.normal')) . ' (' . $this->numberToHumanSize($file_sample['size']) . ' ' . strtoupper($file_sample['ext']) . ')', $file_sample['url'], [
              'class' => $this->post->has_sample() ? "original-file-changed" : "original-file-unchanged",
              'id' => 'highres']);
      else:
          echo $this->linkTo(($this->post->has_sample() ? $this->t('.download.larger') : $this->t('.download.image')) . ' (' . $this->numberToHumanSize($file_jpeg['size']) . ' ' . strtoupper($file_jpeg['ext']) . ')', $file_jpeg['url'], [
              'class' => ($this->post->has_sample() ? "original-file-changed" : "original-file-unchanged"),
              'id' => 'highres']);
      endif
  ?>
      ...
      <?php if ($this->post->has_jpeg()) : ?>
        ...
        <li><?= $this->linkTo($this->t('.download.normal') . ' ' . strtoupper($file_image['ext']) . ' (' . $this->numberToHumanSize($file_image['size']) . ')', $file_image['url'], [
    ...
    <li><?= $this->linkTo($this->t('.delete'), ['#delete', 'id' => $this->post->id]) ?></li>
    <?php endif ?>
    ...
    <?php if (!$this->post->is_flagged() && !$this->post->is_deleted()) : ?>
      <li><?= $this->linkToFunction($this->t('.flag'), "Post.flag(" . $this->post->id . ", function() { window.location.reload(); })", ['level' => 'member']) ?></li>
    <?php endif ?>
    <?php if (!$this->post->is_deleted() && $this->post->image() && !$this->post->is_note_locked()) : ?>
      ...
      <li style="position:relative">
        ...
        <?= $this->linkToFunction($this->t('.add_notes'), "Note.toggleCreateNotice()", ['level' => 'member']) ?>
      </li>
      <?php else: ?>
      <li><?= $this->linkToFunction($this->t('.add_notes'), "Note.create(" . $this->post->id . ")", ['level' => 'member']) ?></li>
      <?php endif ?>
    <?php endif ?>
    ...
    <?php endif ?>
    <?php if (!$this->post->is_deleted()) : ?>
      <li id="set-avatar"><?= $this->linkTo($this->t('.set_avatar'), ['user#set_avatar', 'id' => $this->post->id]) ?></li>
    <?php endif ?>
    <li><?= $this->linkTo($this->t('.history'), ['history#index', 'search' => 'post:' . $this->post->id]) ?></li>
    <?php if (CONFIG()->enable_find_external_data && current_user()->is_mod_or_higher()) : ?>
    <li><?= $this->linkTo('Search external data', ['post#search_external_data', 'ids' => $this->post->id]) ?></li>
    <?php endif ?>
  </ul>
</div>
